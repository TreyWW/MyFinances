...
class UserSettingsViewTestCase(ViewTestCase):
    def test_usersettings_view_302_for_non_authenticated_users(self):
        response = self.client.get(reverse("core:settings:dashboard with page", args=("profile",)))
    ...
    def test_usersettings_view_matches_with_urls_view(self):
        ...
        self.assertEqual("/dashboard/settings/profile/", reverse("core:settings:dashboard with page", args=("profile",)))
        self.assertEqual("core.views.settings.view.view_settings_page_endpoint", func_name)
