...
def generate_chart(income_data:list=[], expenses_data:list=[]): 
    ...
    # Create Bokeh figure
    p = figure(x_range=categories, height=300, title=None,
            toolbar_location=None, tools="")

    # Plot income and expenses as grouped bars
    p.vbar(x=dodge('categories', -0.18, range=p.x_range), top='income_data', 
        width=0.3, line_color="rgba(75, 192, 192, 1)", 
        fill_color="rgba(75, 192, 192, 0.3)", legend_label="Income", source=source)

    p.vbar(x=dodge('categories', 0.2, range=p.x_range), top='expenses_data', 
        width=0.3, line_color="rgba(255, 99, 132, 1)", 
        fill_color="rgba(255, 99, 132, 0.3)", legend_label="Expenses", source=source)
